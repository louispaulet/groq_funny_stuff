...
.PHONY: venv install run clean chat pokedex allergyfinder test
...
chat pokedex allergyfinder:
	@:

install: venv
	@if [ "$(APP)" != "chat" ] && [ "$(APP)" != "pokedex" ] && [ "$(APP)" != "allergyfinder" ]; then \
	  echo "Usage: make install chat|pokedex|allergyfinder"; exit 1; \
	fi
...
run:
	@if [ "$(APP)" != "chat" ] && [ "$(APP)" != "pokedex" ] && [ "$(APP)" != "allergyfinder" ]; then \
	  echo "Usage: make run chat|pokedex|allergyfinder"; exit 1; \
	fi
	...
	@if [ "$(APP)" = "allergyfinder" ]; then cd allergyfinder && npm run dev -- --host --port 5173; fi

test:
	@if [ "$(APP)" != "chat" ] && [ "$(APP)" != "pokedex" ] && [ "$(APP)" != "allergyfinder" ]; then \
	  echo "Usage: make test chat|pokedex|allergyfinder"; exit 1; \
	fi
